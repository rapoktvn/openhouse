...
export const wishas = () => {
    ...
    const listItemComentar = (data) => {
        ...
        if (newDate.days < 1) {
            if (newDate.hours < 1) {
                ...
                date = `${newDate.hours} hr, ${newDate.minutes} min ago`;
            }
        } else {
            date = `${newDate.days} days, ${newDate.hours} hr ago`;
        }
    ...
    const initialComentar = async () => {
        ...
        try {
            ...
            if (comentar.length > 0) {
                peopleComentar.textContent = `${comentar.length} people have sent their wishes`;
            } else {
                peopleComentar.textContent = `No wishes yet — be the first!`;
            }
    ...
    form.addEventListener('submit', async (e) => {
        e.preventDefault();
        buttonForm.textContent = 'Sending...';

        const comentar = {
            id: generateRandomId(),
            name: e.target.name.value,
            status: e.target.status.value === 'y' ? 'Attending' : 'Not Attending',
        ...
        try {
            ...
            peopleComentar.textContent = `${++response.comentar.length} people have sent their wishes`;
